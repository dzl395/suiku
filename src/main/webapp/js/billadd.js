...
$(function(){
    ...
    billCode.on("blur",function(){
        if(billCode.val() != null && billCode.val() !== ""){
            ...
            validateTip(billCode.next(),{"color":"red"},imgNo+" ",false);
        }
    }).on("focus",function(){
        //显示友情提示
        validateTip(billCode.next(),{"color":"#666666"},"* 请输入水库序号",false);
    }).focus();

    productName.on("focus",function(){
        validateTip(productName.next(),{"color":"#666666"},"* 请输入水库名称",false);
    }).on("blur",function(){
        if(productName.val() != null && productName.val() !== ""){
            validateTip(productName.next(),{"color":"green"},imgYes,true);
        }else{
            validateTip(productName.next(),{"color":"red"},imgNo+" ",false);
        }

    });

    productUnit.on("focus",function(){
        validateTip(productUnit.next(),{"color":"#666666"},"* 请输入水库地址",false);
    }).on("blur",function(){
        if(productUnit.val() != null && productUnit.val() !== ""){
            validateTip(productUnit.next(),{"color":"green"},imgYes,true);
        }else{
            validateTip(productUnit.next(),{"color":"red"},imgNo+" ",false);
        }

    });

    providerId.on("focus",function(){
        validateTip(providerId.next(),{"color":"#666666"},"* 请选择承包商",false);
    }).on("blur",function(){
        if(providerId.val() != null && providerId.val() !== "" && providerId.val() !== 0){
            validateTip(providerId.next(),{"color":"green"},imgYes,true);
        }else{
            validateTip(providerId.next(),{"color":"red"},imgNo+" ",false);
        }
