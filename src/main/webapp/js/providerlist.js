	...
	$.ajax({
		...
		success:function(data){
			if(data.delResult === "true"){//删除成功：移除删除行
				...
				changeDLGContent("对不起，删除承包商【"+obj.attr("proname")+"】失败");
			}else if(data.delResult === "notexist"){
				//alert("对不起，供应商【"+obj.attr("proname")+"】不存在");
				changeDLGContent("对不起，承包商【"+obj.attr("proname")+"】不存在");
			}else{
				//alert("对不起，该供应商【"+obj.attr("proname")+"】下有【"+data.delResult+"】条订单，不能删除");
				changeDLGContent("对不起，该承包商【"+obj.attr("proname")+"】下有【"+data.delResult+"】条订单，不能删除");
			}
		},
		error:function(data){
			//alert("对不起，删除失败");
...
$(function(){
	...
	$(".deleteProvider").on("click",function(){
		providerObj = $(this);
		changeDLGContent("你确定要删除承包商【"+providerObj.attr("proname")+"】吗？");
		openYesOrNoDLG();
	});
